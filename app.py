...
# Initialize resolved model in session state
...
def get_api_key() -> str:
    ...
    """
    try:
        # st.secrets is available when configured (e.g., Streamlit Cloud)
        secret_key = st.secrets.get("GOOGLE_API_KEY") if hasattr(st, "secrets") else None
    except Exception:
        secret_key = None
    return (secret_key or os.getenv("GOOGLE_API_KEY", "")).strip().strip('"')
...
if prompt := st.chat_input("Ask me about application packaging..."):
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            if not active_key:
                raise ValueError("No API key configured. Set GOOGLE_API_KEY via Streamlit secrets or environment variables.")
